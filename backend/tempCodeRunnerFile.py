...
def process_speech_from_cloud(file_url):
    ...
    if os.path.exists(wav_audio_path):
        print(f"Found existing WAV file: {wav_audio_path}")
    # Check if M4A file exists and needs conversion
    elif os.path.exists(m4a_audio_path):
        print(f"Found existing M4A file: {m4a_audio_path}")
        print("Converting M4A to WAV...")
        convert_m4a_to_wav(m4a_audio_path, wav_audio_path)
    # If no local files found, download from cloud
    else:
        print("No local files found. Downloading from cloud...")
        download_file_from_cloud(file_url, m4a_audio_path)
        print("Converting downloaded M4A to WAV...")
        convert_m4a_to_wav(m4a_audio_path, wav_audio_path)
